...
head_strings=np.array(["aDtromtu hoCllge eaMtsreR DxPS  ", \
                          "aDtromtu hoCllge elSva eR DxPS  " ])
...
#	struct gray_vals gray; //complex_proc.h
#	struct timeval then; //time.h
#	struct header_info header, head_temp; //complex_proc.h
#
#	double calary[4096], datai;
#	struct cp_opts o; //complex_proc.h
#
#	FILE *tempfile, *specfile, **imagfile, *calfile;
#
#	signal(SIGINT, do_depart);
#
#	init_opt(&o); //complex_proc.h
#	parse_opt(&o, argc, argv); //complex_proc.h
#
...
#	printf("done.\n");
#
#	initialize_dynvar(hlines, vlines, average, n_chan, &o); //complex_proc.h
#
...
#			if (!running) break;
#			usleep(o.granularity); //unistd.h: extern int usleep 
#			memmove(&head_temp, imap, sizeof(struct header_info)); //string.h:extern void *memmove(*__dest, const void *__src, size_t __n)
