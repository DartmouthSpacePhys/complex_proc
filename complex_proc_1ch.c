...
//        printf("searching %li bytes\n",2*header.num_read);
		...
		for (int chan = 0; chan < n_chan; chan++) {
            hptr = memmem(dptr, 2*header.num_read, head_strings[o.endian][0], 32); // Find channel #chan's header
            if (hptr == NULL) {
	      hptr = memmem(dptr, 2*header.num_read, head_strings[o.endian][1], 32);
	      if (hptr == NULL) {
