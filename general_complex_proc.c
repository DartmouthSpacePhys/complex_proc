...
//	tmap = mmap_file("/tmp/rawrtdout", mapsize, O_RDWR|O_TRUNC);
	...
	while (running) {
		...
		memmove(samples, &imap[100], 2 * sizeof(short int) * atotal*n_chan);
//		memmove(tmap, samples, 2 * sizeof(short int) * atotal*n_chan);
...
//			short int *compl = malloc(512*sizeof(short int));
			for (int i = 0; i < atotal; i++) {
				...
				if (o.endian) {
					...
					fft_input[i] =  ((double) endswp(samples[shift+2*i]));
					fft_input[i] += ((double) endswp(samples[shift+2*i+1]))*I;
				}
				else {
					if (o.verbose) printf("No endian switch.\n");fflush(stdout);
					fft_input[i] =  ((double) samples[shift+2*i]);
					fft_input[i] += ((double) samples[shift+2*i+1])*I;
				}
...
void initialize_dynvar(int hlines, int vlines, int average, int n_chan, struct cp_opts *o) {
	...
	if ((s_hl != hlines) || (s_vl != vlines) || (s_av != average) || (s_nc != n_chan)) {
		...
		mapsize = 2*sizeof(short int) * s_hl*s_nc*s_av + 100;
		imap = mmap_file(o->infile, mapsize, O_RDWR);
		samples = malloc(2 * s_hl * s_av * s_nc *sizeof(short int));
...
int parse_opt(struct cp_opts *options, int argc, char **argv) {
    ...
    while (-1 != (c = getopt(argc, argv, "m:f:F:c:g:Evh"))) {
		switch (c) {
			case 'm':
